...
final class MenuBarIconService {
    ...
    private init?() {
        ...
        isVisibleObservation = settingsService.$isMenuBarIconEnabled.sink {
            [correspondingItem] isVisible in
            DispatchQueue.main.async {
                correspondingItem.isVisible = isVisible
            }
        }

        isProtectionActiveObservation = protectionService.$status.sink {
            [correspondingItem] status in
            let image: NSImage
    ...
    private func createProtectionMenuItem() -> NSMenuItem {
        ...
        NSLayoutConstraint.activate([
            titleLabel.leadingAnchor.constraint(
                equalTo: containerView.leadingAnchor, constant: insets.horizontal),
            ...
            switchView.trailingAnchor.constraint(
                equalTo: containerView.trailingAnchor, constant: -insets.horizontal),
            switchView.topAnchor.constraint(
                equalTo: containerView.topAnchor, constant: insets.vertical),
            switchView.bottomAnchor.constraint(
                equalTo: containerView.bottomAnchor, constant: -insets.vertical),
        ])
